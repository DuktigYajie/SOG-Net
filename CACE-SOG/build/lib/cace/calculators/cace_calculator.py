...
class CACECalculator(Calculator):
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        for prop in self.properties:
            energy_key = prop.get("energy", "energy")
            forces_key = prop.get("forces", "forces")
            weight = prop.get("weight", 1.0)
            
            # 提取当前子网络的输出
            energy = to_numpy(output[energy_key])
            forces = to_numpy(output[forces_key])
            
            # 加权累加
            total_energy += energy * weight
            total_forces += forces * weight
        ...
        if self.atomic_energies:
            ...
            e0 = 0.0
        self.results["energy"] = (total_energy + e0) * self.energy_units_to_eV
        self.results["forces"] = total_forces * self.energy_units_to_eV / self.length_units_to_A
...
class CACECalculator1(Calculator):
    """CACE ASE Calculator
    args:
        model_path: str or nn.module, path to model
        device: str, device to run on (cuda or cpu)
