...
class TrainingTask(nn.Module):
    def __init__(self, 
        ...
        #self.loss_list=[]
        self.min_loss = 10000
        self.train_MAE_e = 2.5e-3#1e-3#
        self.train_RMSE_e = 2.5e-3#1e-3#
        self.test_MAE_e = 1e-3#1e-3#
        self.test_RMSE_e = 1e-3#1e-3#
        self.train_MAE_f = 1e-3
        self.train_RMSE_f = 1e-3
        self.test_MAE_f = 1e-3
        self.test_RMSE_f = 1e-3
    ...
    def retrieve_metrics(self, subset, print_log: bool = False):
        ...
        if self.save_folder != None and self.time_name!=None:
            with open(self.save_folder+'rmse'+self.time_name+'.csv','a', newline="") as file:
    ...
    def train_step(self, 
        ...
        if self.save_folder != None and self.time_name!=None:
            with open(self.save_folder+'loss'+self.time_name+'.csv','a') as f:
                f_csv = csv.writer(f)
            ...
            if loss < self.min_loss:
                self.min_loss= loss
                try:
                    #self.save_model(self.save_folder+'loss'+self.time_name+'_model.pth')
                    torch.save(self.model.to(self.device), self.save_folder+'loss'+self.time_name+'_model.pth')
                    try:
                        print("shift:", self.model.models[1].output_modules[1].shift_1)
                        print("weight:", self.model.models[1].output_modules[1].amplitude_1)
                    except (AttributeError, IndexError, KeyError):  # 捕获明确的异常类型
                        # 回退到 models[0]
                        print("shift:", self.model.models[0].output_modules[1].shift_1)
                        print("weight:", self.model.models[0].output_modules[1].amplitude_1)
    ...
    def fit(self, 
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch % val_stride == 0:
                ...
                if (train_mae[0] < self.train_MAE_e) and (test_mae[0]<self.test_MAE_e):
                    self.train_MAE_e = train_mae[0]
                    self.test_MAE_e = test_mae[0]
                    torch.save(self.model.to(self.device), self.save_folder+'min_mae_e'+self.time_name+'_model.pth')
                    print("Save best MAE e model:",self.train_MAE_e,self.test_MAE_e )

                if (train_mae[1] < self.train_MAE_f) and (test_mae[1]<self.test_MAE_f):
                    self.train_MAE_f = train_mae[1]
                    self.test_MAE_f = test_mae[1]
                    torch.save(self.model.to(self.device), self.save_folder+'min_mae_f'+self.time_name+'_model.pth')
                ...
                if (train_rmse[0] < self.train_RMSE_e) and (test_rmse[0]<self.test_RMSE_e):
                    ...
                    torch.save(self.model.to(self.device), self.save_folder+'min_rmse_e'+self.time_name+'_model.pth')
                ...
                if (train_rmse[1] < self.train_RMSE_f) and (test_rmse[1]<self.test_RMSE_f):
                    ...
                    torch.save(self.model.to(self.device), self.save_folder+'min_rmse_f'+self.time_name+'_model.pth')
